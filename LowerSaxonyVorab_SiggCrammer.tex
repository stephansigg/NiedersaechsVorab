...
We plan to apply the machine learning approach in which prediction and feedback tools are built based on data. Specifically, we plan to install and employ a wide range of sensors, some on-body, some are already installed on everyday smartphones, and some are located in an environmental. We will collect data from these sensors and analyze it, where the goal is to find a small, cheap and efficient subset of sensors which provides a robust and stable signal for prediction. Next, we will build a prediction system of sentiment based on this signal. We will use the collected signals and true sentiment to generate with a predictive model of sentiment from signal. We will test the model on signals coming from participants of our study. We will also develop a feedback algorithm that will provide the user information about current sentimental state, and possibly future predicted one. Hopefully, the user will benefit from such feedback. Additional component we plan to pursue is the development of an efficient prediction and feedback methods, that could be executed on a smartphone.

We hypothesize that we will be able to collect sensor information on users, and their true or approximated sentimental state. We also assume that it is possible to predict sentiment from signals of sensors. 
...
\begin{tabular}{rl}
 ...
 Precondition & --\\
 Milestone & \begin{minipage}[t]{12.2cm}
\textit{Sentiment sensing system with interfaces for various sensor types; Android mobile sensing application}\vspace{.2cm}
             \end{minipage}
\end{tabular}
...
\paragraph*{WP-3.2: Data collection from smartphones}
\begin{tabular}{rl}
 Estimated effort& 6 PMs\\
 Precondition & WP-2\\
 Milestone & \begin{minipage}[t]{12.2cm}
...
In addition, in this workpackage, we focus the analysis of sentiment over an extended period of time. 
...
\noindent
In this workpackage, we consider the generation of user feedback.
In particular, we intend to influence the user's sentiment with such feedback. 
Examples are the enforcement of stress-reducing behaviour or recommendations on behaviour to improve health or well-being. 
% \todo
...
%  \item[\color{blue}tbd]
% \end{itemize}
This project will generate an initial body of knowledge on the recognition of sentiment from various sensor types.
